...
@objc protocol YTEmoticonCellDelegate: NSObjectProtocol {
    /// 表情 cell 选中表情模型
    ///
    /// - parameter em: 表情模型／nil 表示删除
    @objc optional func emoticonCellDidSelectedEmoticon(cell:YTEmoticonCell,em:YTEmoticon?)
}
...
/// - 最后一个位置放置删除按钮
class YTEmoticonCell: UICollectionViewCell {
    
    /// 代理
    weak var delegate: YTEmoticonCellDelegate?
    
    /// 当前页面的表情模型数组，`最多` 20 个
    var emoticons:[YTEmoticon]? {
        didSet {
            // 1. 隐藏所有的按钮
    ...
    /// 表情选择提示视图
    private lazy var tipView = YTEmoticonTipView()
    ...
    @objc private func longGesture(gesture: UILongPressGestureRecognizer) {
        ...
        case .began,.changed:
            ...
            // 设置提示视图的表情模型
            if button.tag < emoticons?.count ?? -1 {
...
// MARK: - 设置界面
private extension YTEmoticonCell {
    ...
    func setupUI() {
        ...
        // 设置图像
        let image = UIImage(named: "compose_emotion_delete_highlighted", in: YTEmoticonManager.shared.bundle, compatibleWith: nil)
